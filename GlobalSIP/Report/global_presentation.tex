...
\begin{frame}{Overview of TIC6636K2H Eight Core SoC}
	\begin{itemize}
		\item \blutxt{Four ARM Cortex A15} operating at \@ \eqn{1.4}GHz
		\item \redtxt{Eight C66x CorePacs} DSP Core Subsystems \@ \eqn{1.2}GHz
		\item \gretxt{32KB L1P and L1D Cache} + \gretxt{1024KB L2 Cache Per CorePac}
		\item 6 MB \blutxt{Multicore Shared Memory (MSM) SRAM Memory} Shared by DSP CorePacs and ARM CorePac
...
\begin{frame}{Implementation of Scheduler Algorithm}
	\begin{itemize}
		\item Each core runs \blutxt{separate copy of SYS-BIOS}, \textit{i.e.}, in homogeneous Asynchronous Multiprocessing (AMP) mode
		...
		\item \redtxt{Avoids the usage of \textit{SharedMem} and \textit{Notify} modules} to achieve the same result
		\item \blutxt{\textit{Inter-Processor Communication (IPC)} module is used to synchronize} the cores upon {BIOS\_Start()} function call
		\item \gretxt{\textit{IPC\_start()}} and \gretxt{\textit{IPC\_attach()}} function calls are used for multi-core synchronization
	\end{itemize}
\end{frame}
...
\end{table}
\begin{itemize}
	\item \redtxt{\eqn{\lambda}} - number of spatial streams used in scheduling method (\gretxt{only dominant streams are considered after sorting singular values})
\end{itemize}
\end{frame}
...
\begin{frame}{Conclusion from Implementation Results}
	\begin{itemize}
		...
		\item Using \blutxt{8 parallel cores} - \gretxt{support \eqn{8} scheduling blocks (SBs)} in \eqn{0.5} msec \redtxt{(well within LTE-A subframe duration of \eqn{1} msec)}
		...
		\item ZYNQ ZC702 - \redtxt{five SVD block on programming logic and scheduling block on ARM} - can support only \eqn{50} users for a \eqn{8 \times 2} system
		\item ZYNQ ZC702 performance degradation is \blutxt{due to the clocking of programming logic - (150MHz) and ARM (667 MHz) only}
	\end{itemize}
\end{frame}
