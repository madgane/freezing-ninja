...
We consider a single-cell downlink MU-MIMO transmission with \eqn{N_T} transmit antennas and \eqn{N_K} users with \eqn{N_R} antenna each. Let \eqn{\mvec{H}{k} \in \mathbb{C}^{N_R \times N_T}} be the channel matrix seen by user \me{k}. Let \me{\kappa = \min(N_R,N_T)} be the rank of channel matrix \me{\mvec{H}{k}}. To utilize the available spatial streams, the channel matrix \eqn{\mvec{H}{k}} is decomposed into \me{N_R} virtual single receive antenna channel vectors using singular value decomposition (SVD) as \eqn{\mvec{H}{k} = \mvec{U}{k} \mvec{D}{k} \mvec{V}{k}^\herm}, where \eqn{\mvec{U}{k} \mvec{U}{k}^\herm = \mbf{I}_{N_R}} and \eqn{\mvec{V}{k}\mvec{V}{k}^\herm = \mbf{I}_{N_T}} are unitary matrices and \eqn{\mvec{D}{k}} is a diagonal matrix. The virtual user index for the \me{\ith{i}} column vector of \me{\mvec{U}{k} \in \mathbb{C}^{N_R \times N_R}} of the \me{\ith{k}} user is denoted by \me{\hat{k} = k N_R + i} and the equivalent single receive antenna channel is \eqn{\mvec{h}{\hat{k}} = \mvec{U}{k}(i)^\herm \mvec{H}{k}}. The total number of virtual single antenna users available in the system is given by \eqn{K = N_R N_K}. Let \eqn{\mc{U}} be the set of indices of users present in the system and let \eqn{\mc{A} \subset \mc{U}} be the set of active users chosen by the scheduling algorithm for a transmission instant. For brevity, the virtual users are referred as users in the forthcoming discussion. Now, the equalized received symbol \eqn{\hat{d}_k} of user \me{k \in \mc{A}} for an active spatial stream is given by
\begin{equation}
	\hat{d}_k = \mvec{h}{k} \mvec{m}{k} d_k + \sum_{i \in \mc{A} \backslash \{k\}} \mvec{h}{k} \mvec{m}{i} d_i + {n}_{k},
\end{equation}
where \eqn{{n}_{k} \in \mathbb{C}} is the equivalent zero mean complex Gaussian noise with variance \eqn{\mc{N}(0,N_0)} and \me{\mvec{m}{k} \in \mathbb{C}^{N_T \times 1}} is the transmit precoder corresponding to user \me{k}. Note that the channel vector \eqn{\mvec{h}{k} = \mvec{U}{\tilde{k}}^\herm (i) \mvec{H}{\tilde{k}}}, where \eqn{\mvec{U}{\tilde{k}}(i)} correspond to the \eqn{\ith{i}} column vector of the left singular matrix \eqn{\mvec{U}{\tilde{k}}} and \eqn{\tilde{k}} correspond to a user and \eqn{k} is a virtual user. The data symbol \me{d_k} is assumed to be uncorrelated with \me{\mathrm{E}[|d_k|^2] = 1}.
\begin{figure}
	\centering
	\includegraphics[trim=0in 0.25in 0in 0.5in,width=\columnwidth, angle=0]{Algorithm_Model}
...
In this section, we discuss the fixed point implementation of various scheduling algorithms discussed in the Section \ref{sec:sched_discussions} on TI's Keystone-II platform. For the current implementation work, we considered only single SB scenario and utilize the available \eqn{N_C} multiple cores in the evaluation platform. 

\subsection{SVD Implementation}
The SVD is performed by repeated QR factorization to obtain the left and the right singular vectors of the channel matrices. The complex QR decomposition is performed using the Householder transformations \cite{aft}. We perform \eqn{16} QR factorization for each SVD processing to limit the off diagonal values of the \eqn{\mvec{D}{k}} matrix \eqn{\leq 1e^{-3}}. The SVD incurs significant computational complexity and time, it is the ideal candidate to perform in parallel. Since the matrix size is typically limited by the number of transmit and the receive antennas, parallelizing each SVD operation is not an efficient approach due to the overhead involved in setting up the resources for sharing and synchronization. In the current approach, each TI C66x core shares \eqn{\frac{1}{N_C}} of the available number of SVD operations, which correspond to the actual user count \eqn{N_K} in the system.
	
The channel matrices are stored in \ac{MSMCSRAM}, which are accessible from all cores in the \ac{SoC}. In this work, only Core(0) is used to perform scheduling and all other cores are utilized for SVD processing only. Due to the availability of the shared memory \ac{MSMCSRAM} region, we can synchronize the cores effectively by using shared memory and \ac{IPC} interrupts available through \ac{CIC} instead of using OpenMP framework \cite{openmp,ipc,cic}. Since each core has different operating system (SYS-BIOS), and therefore the core dependent memory addressing will be different for the same region in the shared memory. In order to override this aliasing, we fix the channel buffer to a fixed physical address using the compiler directive \textit{location} in order to be coherent with the channel buffer location. The channels are randomly generated by the Core(0) and is flushed back to the shared memory \ac{MSMCSRAM} before notifying all cores about the channel availability through an \ac{IPC} interrupt. Before proceeding for SVD processing, all cores will invalidate the channel buffer to have the updated channels in the system.

At this point, each core will determine the starting offset in the buffer based on the number of cores sharing the computation and the current core number. If the total number of channel matrices is not an exact multiple of the number of cores, then the first \eqn{\textrm{mod} (N_K,N_C)} processors will perform an additional SVD processing each to share the load. Since only the Core(0) is used to perform the scheduling of users for a given SB, instead of waiting for the interrupt from the other cores for completion, it can proceed ahead to perform user scheduling by invalidating the virtual channel region of all users in the system. Meanwhile, the other cores will complete and write back all the virtual channel matrices to the shared \ac{MSMCSRAM} from the cache before switching to idle task. Cache coherency is maintained in Core(0) by checking the pending interrupt from the other cores only when the user channel matrix decomposition is performed from the other cores. In this way, we can perform efficient use of multi-core subsystem. In our current implementation, we notify the availability of the channels using \ac{CIC} interrupt to all cores. 

\subsection{Scheduler Implementation on C66x}
The scheduling algorithms and the overall control is performed by Core(0). Since we analyze only single scheduling block for the evaluation purposes, we utilize only a C66x Core(0) to perform the scheduler processing. To reduce the latency involved in the computations, we resort to the fixed point implementation of the scheduler algorithms on the PS. The complex baseband channel is represented in a packed real and imaginary format with \eqn{16} bit representation by allocating \eqn{15} bits for the fractional part and one bit to represent the sign. The null space matrix computation requires more cycles in comparison with the other computations involved in the scheduler algorithm. In this procedure, we invalidate the L1 cache lines of Core(0) as well before proceeding with the scheduler algorithm to identify the actual time required to perform the scheduling procedure. The complexities of various schemes are tabulated in Table \ref{table:compexity_comparison}. Note that the overall complexity involved in the scheduler implementation can be reduced by limiting the stream search to the dominant streams only, since the singular values are sorted in the descending order. Due to the available multi-user diversity, limiting the stream search has marginal impact on the sum rate performance. 
\begin{table} \caption{Scheduling Complexity for $N_K = 100$ users ($\mathrm{msec}$) with C66x operating at \eqn{1.2}GHz} \begin{center} \begin{tabular}{c c c c c c c}
...
Table \ref{table:compexity_comparison} compares the complexity of various algorithms using the time as the metric for \eqn{N_K = 100} users. The legend \me{\lambda} in Table \ref{table:compexity_comparison} denotes the number of streams included in the scheduler algorithm search. It can take the maximum value of \me{\kappa}, which is the rank of the channel. As seen from Table \ref{table:compexity_comparison}, significant amount of time is spent on the SVD computations. From Table \ref{table:compexity_comparison}, we can see that the total time taken by the parallel processing of SVD is significantly less than performing on the single core. The factor is slightly less than the number of cores performing SVD. It is due to the fact that the total number of users is not an integer multiple of the core count and also due to the overhead involved in maintaining cache coherency before proceeding for the parallel processing.
